...
class SkillRecipeRepository {
  ...
  // 記住使用者上次在這個 Recipe 的批次安裝畫面選過的目標 agent
  // 下次打開同一個 Recipe 時可以讀回當預設選項
  async updateLastSelectedAgent(id, userId, agent, executor = db) {
    const result = await executor.query(
      `UPDATE skill_recipe
       SET last_selected_agent = $3, updated_at = now()
       WHERE id = $1 AND user_id = $2
       RETURNING *`,
      [id, userId, agent],
    );
